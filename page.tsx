...
export default function Home() {
  ...
  useEffect(() => {
    // 소리 파일 로드 (가장 안정적인 CDN MP3 사용)
    sounds.current.cafe = new Audio("https://www.soundjay.com/misc/sounds/coffee-shop-1.mp3");
    sounds.current.rain = new Audio("https://www.soundjay.com/nature/sounds/rain-01.mp3");
    
    sounds.current.cafe.loop = true;
    sounds.current.rain.loop = true;

    return () => {
      sounds.current.cafe?.pause();
      sounds.current.rain?.pause();
    };
  }, []);
  ...
  const handleCycle = () => {
    if (mode === 'FOCUS') {
      if (sessionCount < 4) {
        setMode('SHORT_BREAK'); setTimeLeft(5 * 60);
        alert("집중 완료! 5분간 쉬세요.");
      } else {
        setMode('LONG_BREAK'); setTimeLeft(20 * 60);
        alert("4세션 완료! 20분간 긴 휴식을 취하세요.");
      }
    } else {
      if (mode === 'LONG_BREAK') setSessionCount(1);
  ...
  const toggleSound = async (type: string) => {
    ...
    if (currentSound === type) {
      ...
      try {
        // 2. 선택한 소리 재생 (사용자 클릭 이벤트 안에서 실행)
        await sounds.current[type].play();
        setCurrentSound(type);
      } catch (err) {
        console.error("재생 실패:", err);
        alert("브라우저 정책으로 인해, 화면을 한 번 클릭한 뒤 다시 버튼을 눌러주세요!");
      }
    }
  };
  ...
  return (
    <div style={{
      ...
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      fontFamily: 'sans-serif', position: 'relative',
      backgroundImage: `linear-gradient(rgba(0,0,0,0.85), rgba(0,0,0,0.85)), url('https://images.unsplash.com/photo-1501339847302-ac426a4a7cbb?q=80&w=2078')`,
      backgroundSize: 'cover', backgroundPosition: 'center'
    }}>
      {/* 진행바 */}
      <div style={{
        position: 'absolute', bottom: 0, left: 0, right: 0,
        backgroundColor: mode === 'FOCUS' ? 'rgba(140, 80, 50, 0.3)' : 'rgba(50, 80, 140, 0.3)',
        height: `${(1 - timeLeft / (mode === 'FOCUS' ? 25*60 : (mode === 'SHORT_BREAK' ? 5*60 : 20*60))) * 100}%`,
        transition: 'height 1s linear', zIndex: 1
      }} />

      <div style={{ position: 'relative', zIndex: 10, display: 'flex', flexDirection: 'row', gap: '40px', maxWidth: '1100px', width: '95%', padding: '20px' }} className="main-container">
        {/* 타이머 섹션 */}
        <div style={{ flex: 1, backgroundColor: 'rgba(255,255,255,0.02)', backdropFilter: 'blur(40px)', padding: '60px 40px', borderRadius: '60px', border: '1px solid rgba(255,255,255,0.05)', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
            {[1, 2, 3, 4].map(i => (
              <div key={i} style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: i <= sessionCount ? '#f97316' : 'rgba(255,255,255,0.1)' }} />
            ))}
          </div>
          <div style={{ fontSize: '12px', letterSpacing: '5px', opacity: 0.4, marginBottom: '10px', fontWeight: 'bold' }}>{mode} {sessionCount}/4</div>
          <div style={{ fontSize: '160px', fontWeight: 100, marginBottom: '40px', letterSpacing: '-10px', lineHeight: 1 }}>{formatTime(timeLeft)}</div>
          
          <div style={{ display: 'flex', gap: '30px', alignItems: 'center' }}>
            <button onClick={() => {setTimeLeft(25*60); setIsActive(false); setSessionCount(1); setMode('FOCUS');}} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.2)', cursor: 'pointer' }}><RotateCcw size={28}/></button>
            <button onClick={() => setIsActive(!isActive)} style={{ width: '90px', height: '90px', borderRadius: '50%', backgroundColor: '#fff', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
              {isActive ? <Pause size={36} color="#000" fill="#000"/> : <Play size={36} color="#000" fill="#000" style={{ marginLeft: '5px' }}/>}
            </button>
            <div style={{ width: '28px' }} />
          ...
          <div style={{ marginTop: '50px', display: 'flex', gap: '20px' }}>
            <button onClick={() => toggleSound('cafe')} style={{ padding: '20px', borderRadius: '30px', border: '1px solid rgba(255,255,255,0.1)', cursor: 'pointer', backgroundColor: currentSound === 'cafe' ? '#fff' : 'rgba(255,255,255,0.05)', color: currentSound === 'cafe' ? '#000' : '#fff' }}><Coffee size={24} /></button>
            <button onClick={() => toggleSound('rain')} style={{ padding: '20px', borderRadius: '30px', border: '1px solid rgba(255,255,255,0.1)', cursor: 'pointer', backgroundColor: currentSound === 'rain' ? '#fff' : 'rgba(255,255,255,0.05)', color: currentSound === 'rain' ? '#000' : '#fff' }}><CloudRain size={24} /></button>
          </div>
          {currentSound && <div style={{marginTop: '20px', fontSize: '11px', color: '#f97316', letterSpacing: '2px'}}>● AMBIENCE PLAYING</div>}
        </div>

        {/* 할 일 섹션 */}
        <div style={{ width: '380px', backgroundColor: 'rgba(255,255,255,0.02)', backdropFilter: 'blur(40px)', padding: '40px', borderRadius: '60px', border: '1px solid rgba(255,255,255,0.05)', display: 'flex', flexDirection: 'column' }}>
          ...
          <form onSubmit={(e:any) => { e.preventDefault(); if(!newTodo) return; setTodos([{id: Date.now(), text: newTodo, done: false}, ...todos]); setNewTodo(""); }} style={{ display: 'flex', gap: '10px', marginBottom: '25px' }}>
            <input value={newTodo} onChange={(e) => setNewTodo(e.target.value)} placeholder="What to focus on?" style={{ flex: 1, backgroundColor: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '18px', padding: '18px', color: '#fff', outline: 'none', fontSize: '14px' }} />
            <button style={{ width: '55px', borderRadius: '18px', border: 'none', backgroundColor: '#fff', fontWeight: 'bold', cursor: 'pointer' }}>+</button>
          </form>
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {todos.map(todo => (
